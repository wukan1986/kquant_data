...
class MergeDataStock_5min_000905(MergeBar):
    def __init__(self, folder):
        super(MergeDataStock_5min_000905, self).__init__(folder)
        self.bar_size = 300
    ...
    def init_datetime(self):
        df = read_h5_tdx("sh", "000905", 300, __CONFIG_H5_STK_DIR__, __CONFIG_TDX_STK_DIR__,
                         __CONFIG_H5_STK_DIVIDEND_DIR__)

        df = filter_dataframe(df, 'DateTime', None, None, fields=['DateTime'])
        # 可以保存，也可以不保存
        self.datetime = df
        super(MergeDataStock_5min_000905, self).init_datetime()
...
if __name__ == '__main__':
    # 得到50成份股内的各种开高低收等行情
    path = os.path.join(__CONFIG_H5_STK_DIR__, "5min_000905.SH")

    mdf = MergeDataStock_5min_000905(path)
    mdf.merge()
    mdf.hmerge()
    mdf.clear()
